...
data$p201=recode(data$p201, "1=0 ; 2=1")
...
Pseudo R2 cuadrado: 1-(residual deviance/null deviance)
```{r, include=FALSE}
1-(14300/15205)
```
Se multiplica el Pseudo R2 cuadrado para obtener el valor de cuánto explica el modelo propuesto:
```{r, include=FALSE}
0.05951989*100
```
...
Estos hallazgos se pueden complementar con la literatura que nos habla de las desigualdades por género en el área del trabajo en especial en el Perú, ya que este no es problema reciente, y como se había expresado de manera previa, las consecuencias de la pandemia por COVID-19 y las medidas para disminuir su propagación han visibilizado de mayor manera las probelmáticas esstructurales ya existentes. Además de ello, estos resultados  obtenidos pueden ser útiles ante la poca literatura existente que explica el nivel de desempleo y sus causas en el contexto pandémico, esto debido que, al ser una problemática actual, los estudios son precavidos.

A pesar de ello, nuestra hipótesis principal, la cual buscaba averiguar si los factores culturales determinaban el nivel de desempleo no logró ser significativa. Esta la podemos entender a partir de las limitaciones del trabajo, como que al ser un contexto que no ha culminado, el de la pandiam por el COVID-19, y que continuará desarrollándose, puede que estos factores sean más pronunciados en data más actual. Además de ello, sería importante revisar los datos del desempleo a nivel nacional, esto debido a que en Lima Metropolitana y en el Callao, hay más personas que se autoidentifican como mestizos, el cual es mayor a comparación de otras regiones del Perú en donde hay mayorías quechuas o aymaras. Esto sería importante para analizar cómo las variables culturales como la autoidentificación étnica se comportan respecto al desempleo en contextos distintos al limeño.
